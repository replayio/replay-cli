...
test.describe('Switch Component', () => {
  ...
  test('renders switch with description', async ({ mount }) => {
    ...
    await expect(component.getByTestId('switch-description')).toHaveText('Receive email updates about your account');
  });
  ...
  test('size variants work correctly', async ({ mount }) => {
    const component = await mount(
      <div className="p-8 space-y-4 bg-white rounded-lg">
        <Switch size="sm" defaultChecked={false}>Small switch</Switch>
        <Switch size="md" defaultChecked={false}>Medium switch</Switch>
        <Switch size="lg" defaultChecked={false}>Large switch</Switch>
      </div>
    );
  ...
  test('disabled states work correctly', async ({ mount, page }) => {
    const component = await mount(
      <div className="p-8 space-y-4 bg-white rounded-lg">
        <Switch disabled={true} defaultChecked={false}>Disabled Unchecked</Switch>
        <Switch disabled={true} defaultChecked={true}>Disabled Checked</Switch>
      </div>
    );
  ...
  test('switch with description disabled state', async ({ mount }) => {
    ...
    await expect(component.getByTestId('switch-description')).toHaveText('This switch is disabled and cannot be toggled');
  ...
  test('visual regression test - size variants', async ({ mount }) => {
    const component = await mount(
      <div className="p-8 space-y-6 bg-white rounded-lg">
        <Switch size="sm" defaultChecked={false}>Small</Switch>
        <Switch size="md" defaultChecked={true}>Medium</Switch>
        <Switch size="lg" defaultChecked={false}>Large</Switch>
      </div>
    );
