...
import dbg from "@replayio/dumpable-debug";
...
import { assertValidBrowserName, fuzzyBrowserName, getDirectory } from "./utils";
...
const ctx = { debug: dbg("replay:cli", { outputDir: getDirectory() }) };
  ...
  return program
    ...
    .hook("preAction", async cmd => {
      try {
        ...
        ctx.debug("LaunchDarkly profile is anonymous %o", e);
      }
    });
}
...
async function exitCommand(ctx: Ctx, exitCode: number, warn = false) {
  await getLaunchDarkly(ctx).close();
  console.error("\n📄 Additional logs available in", await ctx.debug.closeLogFile(), "\n");
  if (exitCode) {
    process.exit(warn ? 0 : exitCode);
    return;
  }
  process.exit(0);
}
...
async function commandListAllRecordings(
  ...
  try {
    ...
    await exitCommand(ctx, 0);
  } catch (e) {
    console.error("Failed to list all recordings");
    ctx.debug("removeRecording error %o", e);

    await exitCommand(ctx, 1, opts.warn);
  }
}
...
async function commandUploadRecording(id: string, opts: CommandLineOptions) {
  try {
    ctx.debug("Options", opts);
    const recordingId = await uploadRecording(id, { ...opts, verbose: true }, ctx);
    await exitCommand(ctx, recordingId ? 0 : 1);
  } catch (e) {
    console.error("Failed to upload recording");
    ctx.debug("uploadRecording error %o", e);
...
async function commandLaunchBrowser(
  ...
  try {
    ...
    ctx.debug("launchBrowser error %o", e);
...
async function commandProcessRecording(id: string, opts: CommandLineOptions) {
  try {
    ...
    const recordingId = await processRecording(ctx, id, { ...opts, verbose: true });
    await exitCommand(ctx, recordingId ? 0 : 1, opts.warn);
  } catch (e) {
    console.error("Failed to process recording");
    ctx.debug("processRecording error %o", e);
    await exitCommand(ctx, 1, opts.warn);
  }
}
...
async function commandUploadAllRecordings(opts: CommandLineOptions & UploadAllOptions) {
  try {
    ...
    await exitCommand(ctx, uploadedAll ? 0 : 1, opts.warn);
  } catch (e) {
    console.error("Failed to upload all recordings");
    ctx.debug("uploadAllRecordings error %o", e);
    await exitCommand(ctx, 1, opts.warn);
  }
}
...
async function commandViewRecording(id: string, opts: CommandLineOptions) {
  try {
    ...
    await exitCommand(ctx, viewed ? 0 : 1, opts.warn);
  } catch (e) {
    console.error("Failed to view recording");
    ctx.debug("viewRecording error %o", e);
    await exitCommand(ctx, 1, opts.warn);
  }
}
...
async function commandViewLatestRecording(opts: CommandLineOptions) {
  try {
    ...
    console.error("Failed to view recording");
    ctx.debug("viewLatestRecording error %o", e);
    await exitCommand(ctx, 1, opts.warn);
  }
}
...
async function commandRemoveRecording(
  ...
  try {
    ...
    await exitCommand(ctx, removed ? 0 : 1, opts.warn);
  } catch (e) {
...
async function commandRemoveAllRecordings(opts: Pick<CommandLineOptions, "directory" | "warn">) {
  try {
    ...
    ctx.debug("removeAllRecordings error %o", e);
    await exitCommand(ctx, 1, opts.warn);
  }
}
...
async function commandUpdateBrowsers(
  ...
  try {
    ...
    ctx.debug("updateBrowser error %o", e);
...
async function commandUploadSourcemaps(
  ...
  opts: SourcemapUploadOptions & Pick<CommandLineOptions, "apiKey" | "warn">
): Promise<void> {
  ctx.debug("Options", opts);

  const { quiet, verbose, apiKey, batchSize, warn, ...uploadOpts } = opts;
  ...
  try {
    ...
    ctx.debug("uploadSourceMaps error %o", e);
...
async function commandMetadata(opts: MetadataOptions & FilterOptions) {
  try {
    ...
    ctx.debug("updateMetadata error %o", e);
...
async function commandLogin(opts: CommandLineOptions) {
  try {
    const ok = await maybeAuthenticateUser(ctx, {
      ...opts,
      verbose: true,
    });
    await exitCommand(ctx, ok ? 0 : 1, opts.warn);
  } catch (e) {
    console.error("Failed to login");
    ctx.debug("maybeAuthenticateUser error %o", e);
    await exitCommand(ctx, 1, opts.warn);
  }
}
...
async function commandVersion(opts: CommandLineOptions) {
  try {
    ...
    ctx.debug("commandVersion error %o", e);
    await exitCommand(ctx, 1, opts.warn);
  }
}
