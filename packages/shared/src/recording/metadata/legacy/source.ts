...
async function expandCommitMetadataFromGitHub(repo: string, sha?: string) {
  ...
  logger.debug(`Fetching commit metadata from ${url} with ${GITHUB_TOKEN?.length || 0} char token`);
...
async function expandMergeMetadataFromGitHub(repo: string, pr?: string) {
  ...
  logger.debug(`Fetching merge metadata from ${url} with ${GITHUB_TOKEN?.length || 0} char token`);
